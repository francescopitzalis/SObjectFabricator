...
```sh
sf package create --name "SObjectFabricator" --package-type Unlocked --path force-app
```
...
```sh
sf package version create --package "SObjectFabricator" --definition-file config/project-scratch-def.json --wait 10 --installation-key-bypass --code-coverage
```
...
```
E.g. `sf package version promote --package "SObjectFabricator@1.0.0-1"`
...
```
E.g. `sf package install --package "SObjectFabricator@1.0.0-2" --target-org MyProdOrg --wait 10 --publish-wait 10`
